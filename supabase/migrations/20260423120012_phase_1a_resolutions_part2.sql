-- ============================================================================
-- 0012_phase_1a_resolutions_part2.sql  (part 2 of 2)
--
-- Continuation of 0011. Contains every Phase 1A resolution that references
-- the `closed_awaiting_invoice` enum value OR is schema-logically grouped
-- with the set. Split from 0011 because ALTER TYPE ... ADD VALUE cannot
-- commit in the same transaction as any statement that uses the new value.
--
-- 1. Pre-advance purchase_order_id nullable (advance_type='pre_advance' only)
-- 2. mv_client_position rebuilt to:
--    - include closed_awaiting_invoice in active PO value pool
--    - compute pre-advance borrowing base from eligible (non-aged-out) AR
-- 3. invoices.invoice_date SET NOT NULL
-- 4. one_time_fees polymorphic target validation trigger
-- ============================================================================
...
SELECT
  ...
  -- Pre-advance base uses ELIGIBLE (non-aged-out) AR principal, not raw.
...
-- ---------- 4. One-time fee polymorphic target validation ----------
