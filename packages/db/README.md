...
| `0022` | `po_cancellation_events` | Extends `bulk_upsert_purchase_orders` to emit `po_cancelled` events when an existing PO transitions from a non-cancelled status into `cancelled` (one event per affected PO), and `po_cancellation_reversed` for the reverse direction when a prior unreversed `po_cancelled` exists to reference. Per Derek's principle: balance-affecting changes belong on the ledger so the timeline is reconstructable from events alone. Captures pre-upsert status via a small jsonb snapshot prior to the INSERT … ON CONFLICT, then post-state diff in CTEs. NEW PO inserts with status='cancelled' on first appearance don't emit (never contributed to BB). |
